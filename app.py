                    ...
                    if st.button(f"Registrar uso de {row['code']}", key=f"use_{row['code']}"):
                        if qty > 0 and user.strip() and code.strip():
                            if code == VALID_CODE:
                                ...
                                # Actualizar locations.csv con validación
                                loc_df = pd.read_csv("locations.csv").fillna("")
                                if "available" in loc_df.columns:
                                    if row['code'] in loc_df['code'].values:
                                        current_available = int(loc_df.loc[loc_df['code'] == row['code'], 'available'].values[0])
                                        if qty <= current_available:
                                            loc_df.loc[loc_df['code'] == row['code'], 'available'] = current_available - qty
                                            ...
                                            st.error(f"No se puede registrar: solicitó {qty}, pero solo hay {current_available} disponibles.")
                                    else:
